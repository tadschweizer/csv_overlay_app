...
if uploaded:
    ...
    if unknown_count:
        st.warning(f"{unknown_count} file(s) will be skipped unless you assign SF or DUR.")

    known_files = [f.name for f in uploaded if file_type_overrides.get(f.name) in ("SF", "DUR")]
    ...
    if dur_first_last_rows:
        ...
        st.dataframe(df_dur.style.background_gradient(subset=["% Change (First→Last)"], cmap="RdYlGn"), use_container_width=True)
        st.download_button(
